...
def test_full_pipeline_critical_path() -> None:
    ...
    try:
        ...
        customer_count = con.execute(
            "SELECT COUNT(*) FROM main_intermediate.int_unified_customers"
        ).fetchone()[0]
        ...
        customer_cols = {
            ...
            for row in con.execute(
                "DESCRIBE main_intermediate.int_unified_customers"
            ).fetchall()
        }
        ...
        product_count = con.execute(
            "SELECT COUNT(*) FROM main_intermediate.int_unified_products"
        ).fetchone()[0]
        ...
        product_cols = {
            row[0]
            for row in con.execute(
                "DESCRIBE main_intermediate.int_unified_products"
            ).fetchall()
        }
